...
const CardList = async ({ title }: { title: string }) => {
  ...
  if (title === "Popular Products") {
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_PRODUCT_URL}/products?limit=5&popular=true`);
      if (res.ok) {
    ...
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_ORDER_URL}/orders?limit=5`, {
        headers: {
        ...
        {title === "Popular Products"
          ...
          : orders.map((item) => (
              ...
              >
                ...
                <CardFooter className="p-0">$ {Number(item.amount) / 100}</CardFooter>
              </Card>
            ))}
      </div>
    </div>
